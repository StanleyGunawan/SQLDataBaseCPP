#ifndef TOKENIZER_STATE_MACHIHNE_FUNCTION_CPP
#define TOKENIZER_STATE_MACHIHNE_FUNCTION_CPP
#include "state_machine_function.h"
    
    void init_table(int _table[][MAX_COLUMNS]){
        for (int i = 0; i < MAX_ROWS; i++){
            for (int j = 1; j < MAX_COLUMNS; j++){          //maybe change column to 30 / row to 127?
                _table[i][j] = -1;
            }
        }
    }
